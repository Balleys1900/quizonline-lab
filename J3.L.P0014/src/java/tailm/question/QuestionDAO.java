...
public class QuestionDAO implements Serializable {
    ...
    public void loadQuestionDefault(int pageIndex) throws NamingException, SQLException {
        ...
        try {
            ...
            if (con != null) {
                String sql = "SELECT questionID,email, questionContent, createDate, subjectID, status "
                ...
                while (rs.next()) {
                    ...
                    listQuestion.add(new QuestionDTO(rs.getInt("questionID"),
                            rs.getString("email"), rs.getString("questionContent"),
                            rs.getDate("createDate"), rs.getString("subjectID"),
    ...
    public boolean addNewQuestion(QuestionDTO dto) throws SQLException, NamingException {
        ...
        try {
            con = DBHelpers.makeConnection();
            if (con != null) {
                String sql = "INSERT INTO question(email, questionContent, createDate, subjectID, status) "
                        + "VALUES(?, ?, ?, ?, ?)";
                ps = con.prepareStatement(sql);
                ps.setString(1, dto.getEmail());
                ps.setString(2, dto.getQuestionContent());
                ps.setDate(3, dto.getCreateDate());
    ...
    public void loadQuestionQuiz(String subjectID, int numberQuestion) throws NamingException, SQLException {
        ...
        try {
            con = DBHelpers.makeConnection();
            if (con != null) {
                String sql = "SELECT TOP " + numberQuestion + " questionID,email,questionContent,createDate,subjectID,status "
